...
const root=resolve(import.meta.dirname,'..'), out=resolve(root,'docs/codex-v02/V02-003B-evidence');
...
const commands={build:'npm run build:ci',python:'node scripts/run-python.mjs -m pytest -q engine/tests --tb=short --junitxml=docs/codex-v02/V02-003B-evidence/python-junit.xml',offline:'npm run test:offline',assets:'npm run test:assets',pack:'npm pack --json',freeze:'"'+python+'" -m pip freeze --all'};
