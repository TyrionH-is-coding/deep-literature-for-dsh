...
archive=root/'dsh-external-dsh-scientific-reading-0.2.0-dev.2.tgz'
sha=lambda b:hashlib.sha256(b).hexdigest()
source=subprocess.check_output(['git','rev-parse','HEAD'],cwd=root,text=True).strip()
assert not subprocess.check_output(['git','diff','e3f58d1710413b5c6340fe55974a227b97ee8373','--','engine'],cwd=root)
with tarfile.open(archive) as tgz:
    ...
    assert pkg['version']=='0.2.0-dev.2'
...
(root/'docs/codex-v02/V02-003D-evidence/artifact.json').write_text(json.dumps(record,indent=2)+'\n',encoding='utf-8')
