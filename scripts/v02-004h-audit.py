"""004H current-source direct rejection and real module CLI recovery checks."""
...
OUT = REPO/'docs/codex-v02/V02-004H-evidence'
...
def env_for(control):
    ...
    env.update(PYTHONUTF8='1',PYTHONIOENCODING='utf-8',PYTHONNOUSERSITE='1',SR_NATIVE_KEYRING_TEST='0',
               MINERU_EXECUTABLE=str(Path(sys.prefix)/'Scripts/v004h-provider.exe'),V004H_CONTROL=str(control))
    return env
...
def seed(root,name,via_cli=False):
    ...
    control.mkdir()
    data=control/'data'
    meta=PaperMetadata(title='Load Distribution in Modular Truss Bridges',authors=['Alex Rivera'],doi='10.5555/v004h-'+name,year=2026,journal='Fictional Engineering Notes')
...
def env_for_saved(control):
    ...
    env.update(PYTHONUTF8='1',PYTHONIOENCODING='utf-8',PYTHONNOUSERSITE='1',SR_NATIVE_KEYRING_TEST='0',MINERU_EXECUTABLE=str(Path(sys.prefix)/'Scripts/v004h-provider.exe'),V004H_CONTROL=str(control))
    return env

def main():
    parser=argparse.ArgumentParser();parser.add_argument('--expect-direct-safe',action='store_true');args=parser.parse_args()
    root=Path(tempfile.mkdtemp(prefix='h4-', dir='C:/tmp'))
    report={'root':str(root),'python':sys.executable,'importSource': __import__('scientific_reading.reading_pipeline', fromlist=['x']).__file__, 'source_commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=REPO,text=True).strip(),'cases':{},'cleanup':[]}
    try:
        for name in ('direct-none','direct-success','direct-refail','direct-gate','cli-resume','cli-start'):
            ...
            if False: # Historical corruption assertions are replaced below.
            ...
            if name.startswith('direct') and name != 'direct-none':
                assert 'full_read_pipeline_resume_required' in case.get('advance_traceback', ''), case
                assert case['initial']['snapshot']['hashes'] == case['after']['hashes']
        ...
        report['diagnosis']='direct explicit advance rejects before writes; real CLI resume/start retain lifecycle ownership'
    except Exception:
