...
def run(output):
    ...
    changed = subprocess.check_output(['git', 'diff', '--name-only', '776a018c7855442de38d591517eea831eae5b463', 'HEAD', '--', 'engine', 'scripts'], cwd=ROOT, text=True).splitlines()
    changed = [str(p.relative_to(ROOT)) for p in (ROOT / 'engine').rglob('*.py')] + [str(p.relative_to(ROOT)) for p in (ROOT / 'scripts').glob('v02-004h-*.py')]
    hashes = {name: hashlib.sha256((ROOT / name).read_bytes()).hexdigest() for name in changed}
    run_root = Path(tempfile.mkdtemp(prefix='v4h-suite-', dir='C:/tmp')).resolve()
    env = {k: v for k, v in os.environ.items() if k.upper() in {'SYSTEMROOT', 'WINDIR', 'COMSPEC', 'TEMP', 'TMP', 'PATH', 'PATHEXT'}}
    for key in list(env):
