...
spec = importlib.util.spec_from_file_location('probe', Path(__file__).with_name('v02-004a-probe.py'))
...
def main():
    ...
    for attempt in range(12):
        ...
        with (root / f'resume-{attempt}.log').open('w', encoding='utf-8') as log:
            child = subprocess.Popen([sys.executable, str(Path(p.__file__)), '--worker', str(root)],
    ...
    result = dict(parent=parent, final=pipeline.inspect(parent).to_dict(), processes=processes,
                  derived=derived, legacy_checkpoint=checkpoint, assets=p.assets(root))
