...
def worker(root):
    record_provenance(root / ('provenance-' + str(os.getpid()) + '.json'))
...
def scenario(root, name):
    ...
    protected = {}
    env = os.environ.copy()
    # Children use the installed interpreter; no production import overlay.
    ...
    for attempt in range(14):
        ...
        with log.open('w', encoding='utf-8') as out:
            child = subprocess.Popen([sys.executable, '-I', '-X', 'utf8', str(Path(__file__).resolve()), '--worker', str(root)],
...
def record_provenance(path):
    import scientific_reading, reader, importlib.metadata
    site = Path(scientific_reading.__file__).resolve().parent.parent
    assert 'site-packages' in str(site), site
    assert sys.flags.isolated == 1
    files = {str(f.relative_to(site)): sha(f) for package in ('scientific_reading', 'reader') for f in (site/package).rglob('*.py')}
    atomic_write_json(path, dict(executable=sys.executable, python=sys.version, isolated=sys.flags.isolated,
        engine=scientific_reading.__file__, reader=reader.__file__, sys_path=sys.path,
        distributionVersion=importlib.metadata.version('dsh-scientific-reading-engine'), files=files))
...
def main():
    ...
    record_provenance(args.output / 'provenance.json')
